...
from bot.utils import DATE_BUTTON_FORMAT, TIME_BUTTON_FORMAT, format_date_button, format_time_button
...
def dates_keyboard(dates) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=format_date_button(date), callback_data=f"date:{date.strftime(DATE_BUTTON_FORMAT)}")]
        for date in dates
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows or [[InlineKeyboardButton(text="Нет дат", callback_data="noop")]])


def times_keyboard(slots: list[Slot]) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=format_time_button(slot.slot_at), callback_data=f"time:{slot.id}")]
        for slot in slots
    ]
...
def payment_keyboard(slot_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Оплатить", callback_data=f"pay:{slot_id}"),
...
def delete_slots_keyboard(slots: list[Slot]) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=f"{slot.slot_at.strftime('%d.%m.%Y %H:%M')} — {slot.status.label_ru}", callback_data=f"delslot:{slot.id}")]
        for slot in slots
