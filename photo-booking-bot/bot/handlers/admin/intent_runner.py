"""Execute parsed admin voice/text intents."""
...
from bot.utils import BookingStatus, format_slot_datetime, parse_slot_datetime
...
async def execute_admin_intent(
    ...
    if intent == "add_slot":
        ...
        if date_s and time_s:
            ...
            await message.answer(
                "Укажите дату и время или используйте меню «Слоты → Добавить».",
                reply_markup=admin_slots_menu_kb(),
            )
        return
    ...
    await message.answer(
        "Не понял команду. Используйте меню ⚙️ Админка или напишите, например: "
