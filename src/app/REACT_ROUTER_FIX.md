...
- `react-router` (новая версия)

Файл SEOHelmet.tsx использовал `react-router`, но App.tsx и остальные компоненты использовали `react-router-dom`, что приводило к конфликту контекстов Router.

---

## ✅ Решение

### Обновлены все импорты с `react-router-dom` на `react-router`
...
### Используемые хуки:
- ✅ `BrowserRouter` - из react-router
- ✅ `Routes` - из react-router
...
### После исправления:
- ✅ Все компоненты используют единую версию react-router
...
Теперь весь проект использует `react-router` без исключений.
...
**Q: Почему react-router а не react-router-dom?**  
...
A: Всегда используйте `import { ... } from 'react-router'` во всех новых файлах.
...
**Важно:** При создании новых компонентов всегда используйте `react-router` для импортов, чтобы избежать конфликтов версий в будущем!
